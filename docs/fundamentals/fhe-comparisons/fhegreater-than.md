...
/// @title FHE Greater Than - Demonstrates greater-than comparison operations on encrypted values using FHE.gt
/// @notice This example shows how to compare two encrypted values to check if one is greater than the other without decrypting them.
///         The contract demonstrates the FHE comparison operation by checking if the first encrypted uint8 value is greater than the second.
///         The result is an encrypted boolean that can only be decrypted by users who have been granted FHE permissions.
contract FHEGreaterThan is ZamaEthereumConfig {
  euint8 private _a;
  euint8 private _b;
  ebool private _a_gt_b;
  ...
  /// @dev Performs FHE greater-than comparison using FHE.gt, then grants permissions for result decryption
  ///          The operation compares encrypted values and returns an encrypted boolean result.
  function computeAGreaterThanB() external {
    _a_gt_b = FHE.gt(_a, _b);

    FHE.allowThis(_a_gt_b);
    FHE.allow(_a_gt_b, msg.sender);
  }
