...
- `.sol` file → `<your-project-root-dir>/contracts/fundamentals/fhe-boolean-logic/`
- `.ts` file → `<your-project-root-dir>/test/fundamentals/fhe-boolean-logic/`
...
### 1. Converts external encrypted input to internal format and grants contract FHE permissions for value a
...
### 2. Performs FHE NOT operation using FHE.not, then grants permissions for result decryption The operation performs logical NOT on an encrypted boolean value without revealing it.
...
### 1. Encrypts and sets the boolean value with proper input proof

```typescript
const inputA = await fhevm.createEncryptedInput(contractAddress, signers.alice.address).addBool(a).encrypt();
```

### 2. Performs NOT operation and grants decryption permission to caller

```typescript
tx = await contract.connect(bob).computeNotA();
```

### 3. Decrypts the encrypted boolean result using user decryption
...
### 4. Should return true when input is false

```typescript
expect(clearResult).to.equal(true);
```

### 5. Alice doesn't have permissions because bob called computeNotA()
